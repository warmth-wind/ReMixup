...
def main(args):
    ...
    os.makedirs(args.destdir, exist_ok=True)  #创建路径
    ...
    if not args.srcdict and os.path.exists(dict_path(args.source_lang)):  #路径存在返回错误
    ...
    if args.joined_dictionary:
        ...
        if args.srcdict:
            ...
            src_dict = build_dictionary([train_path(args.source_lang)], src=True)  #返回源语言路径 后构建去除低于词频阈值词的字典
    ...
    def make_binary_dataset(vocab, input_prefix, output_prefix, lang, num_workers):
        logger.info("[{}] Dictionary: {} types".format(lang, len(vocab)))  #输出日志
        n_seq_tok = [0, 0]
        replaced = Counter()
        ...
        ds = indexed_dataset.make_builder(
            dataset_dest_file(args, output_prefix, lang, "bin"),   #文件输出路径
